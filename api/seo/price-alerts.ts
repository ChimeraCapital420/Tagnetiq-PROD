...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const deals = (listings || []).filter((l: any) => {
    ...
    return res.status(500).send(`<?xml version="1.0"?><e>${error.message}</e>`);
  }
}
...
function generatePriceAlertsFeed(listings: any[]): string {
  ...
  const items = listings.map(listing => {
    ...
    const discount = Math.round(
      ((listing.estimated_value - listing.asking_price) / listing.estimated_value) * 100
    );
