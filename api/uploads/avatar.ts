...
// FILE: api/uploads/background.ts
// Background upload endpoint

export default async function backgroundHandler(req: VercelRequest, res: VercelResponse) {
  if (req.method !== 'POST') {
    return res.status(405).json({ error: 'Method not allowed' });
  }
  ...
  try {
    ...
    // Similar logic to avatar but for backgrounds
    const fileType = filename.split('.').pop()?.toLowerCase();
    const uniqueFilename = `${user.id}-bg-${Date.now()}.${fileType}`;
    const filePath = `backgrounds/${uniqueFilename}`;
