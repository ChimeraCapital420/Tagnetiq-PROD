...
//
// v11.3: Data Integrity Rule — Added Step 12:
//   Step 12 — Oracle always distinguishes market-verified data from
//             AI estimates. Never states a naked price without source
//             attribution. Builds user trust. Prevents hallucinated
//             prices from being stated as fact.
//
// ZERO LOGIC CHANGES to Steps 1–11. Purely additive.
// ═══════════════════════════════════════════════════════════════════════
...
// =============================================================================
// DATA INTEGRITY BLOCK (v11.3 — Step 12)
// =============================================================================

/**
 * Inject data integrity rule into every Oracle response.
 *
 * Oracle must always distinguish between market-verified data (eBay sold
 * listings, authority APIs) and AI estimates. Never present a guess as a fact.
 * Builds user trust. Prevents hallucinated prices.
 *
 * Cost: $0. ~100 tokens. Always injected.
 */
function buildDataIntegrityBlock(): string {
  return `

================================================================================
DATA INTEGRITY RULE
================================================================================
When citing a price or value, ALWAYS distinguish the source:
- Market-verified: "eBay sold listings show $45-65" or "Numista catalog value: $12"
- AI estimate: "I estimate $30-40 based on condition and category"
- NEVER state a price without attribution. No naked "$50" — always say WHERE
  that number comes from.
- If you are unsure of a value, say so: "I'd guess around $X but scan it for
  market-verified pricing."
- When multiple sources conflict, name them both: "eBay shows $45 but Discogs
  shows $60 — the gap is likely condition-dependent."
================================================================================
`;
}
 ...
 *   12. Inject Data Integrity rule (v11.3)
 *
...
export function assembleSystemPrompt(input: PromptAssemblyInput): string {
  ...
  // ── 12. Data Integrity rule (v11.3) ─────────────────────
  // Always injected. Oracle must distinguish market-verified data from
  // AI estimates. No naked prices. Always attribute the source.
  // ~100 tokens. Builds user trust. Prevents hallucinated pricing.
  systemPrompt += buildDataIntegrityBlock();
