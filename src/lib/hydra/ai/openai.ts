...
// Refactored from 2,300-line monolith to ~250 lines
...
function validateRequest(body: unknown): AnalyzeRequest {
  ...
  if (rawBody.items && Array.isArray(rawBody.items) && rawBody.items.length > 0) {
    ...
    if (primaryItem.name && !primaryItem.name.startsWith('Photo ') && !primaryItem.name.startsWith('Video ')) {
      ...
      // For image-only scans, let AI identify the item
      itemName = 'Identify this item from the image';
    }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // 3. Build images array for AI providers
    const images: string[] = [];
    if (request.imageBase64) {
      images.push(request.imageBase64);
    }
    if (request.additionalImages) {
      images.push(...request.additionalImages.slice(0, 3)); // Limit to 4 total images
    }
    ...
    // 6. Blend prices from all sources
...
async function runMultiAIAnalysis(
  ...
  const visionVotes = await runProviderStage(visionProviders, images, fullPrompt, category);
  votes.push(...visionVotes);
  
  // Extract best context from vision results for text-only models
  const bestContext = votes.length > 0
    ? votes.reduce((a, b) => a.weight > b.weight ? a : b).reasoning || itemName
    : itemName;
  
  // Build text-only prompt with vision context
  const textPrompt = buildAnalysisPrompt({
    categoryHint: categoryHint || category,
    itemNameHint: bestContext,
    additionalInstructions: 'Note: Analyze based on the provided description. No image available for this analysis.',
  });
  ...
  const textVotes = await runProviderStage(textProviders, [], textPrompt, category);
  ...
  if (isProviderAvailable('perplexity')) {
    ...
    const perplexityVotes = await runProviderStage(['perplexity'], [], marketPrompt, category);
  ...
  if (votes.length >= 4 && isProviderAvailable('deepseek')) {
    ...
    if (shouldTrigger) {
      ...
      const tiebreakerVotes = await runProviderStage(['deepseek'], [], tiebreakerPrompt, category);
